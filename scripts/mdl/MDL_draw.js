...
*/
  ...
  const _p3d_wall = function(
    ...
    let
      fBits1 = MDL_color._color(colorIn_gn).toFloatBits(),
      fBits2 = MDL_color._color(colorOut_gn).toFloatBits();
    ...
    Fill.quad(
      x1, y1, fBits1,
      x2, y2, fBits1,
      toP3dCoord(x2, z3d, false), toP3dCoord(y2, z3d, true), fBits2,
      toP3dCoord(x1, z3d, false), toP3dCoord(y1, z3d, true), fBits2,
    );
  ...
  const _p3d_room = function thisFun(
    ...
    let
      colorIn = MDL_color._color(colorIn_gn, Tmp.c1),
      colorOut = MDL_color._color(colorOut_gn, Tmp.c2);

    if(z == null) z = Draw.z();
    _p3d_wall(x - w * 0.5, y - h * 0.5, x + w * 0.5, y - h * 0.5, z3d, Tmp.c2.set(colorIn).mul(0.75), Tmp.c3.set(colorOut).mul(0.75), z - 0.1 + thisFun.calcOff(0, x, y - h * 0.5));
    _p3d_wall(x - w * 0.5, y + h * 0.5, x + w * 0.5, y + h * 0.5, z3d, Tmp.c2.set(colorIn).mul(1.2), Tmp.c3.set(colorOut).mul(1.2), z - 0.1 + thisFun.calcOff(2, x, y + h * 0.5));
    _p3d_wall(x - w * 0.5, y - h * 0.5, x - w * 0.5, y + h * 0.5, z3d, Tmp.c2.set(colorIn), Tmp.c3.set(colorOut), z - 0.1 + thisFun.calcOff(3, x - w * 0.5, y));
    _p3d_wall(x + w * 0.5, y - h * 0.5, x + w * 0.5, y + h * 0.5, z3d, Tmp.c2.set(colorIn), Tmp.c3.set(colorOut), z - 0.1 + thisFun.calcOff(1, x + w * 0.5, y));
  }
  .setProp({
    calcOff: (ind, x, y) => {
  ...
  const _p3d_roomFade = function(
    ...
    let
      colorIn = MDL_color._color(color_gn),
      colorOut = Tmp.c3.set(colorIn);
    colorOut.a = 0.0;
    _p3d_room(x, y, z3d, w, h, colorIn, colorOut, z);
  };
  ...
  const _p3d_reg = function(
    ...
    Draw.rect(reg, toP3dCoord(x, z3d, false), toP3dCoord(y, z3d, true), w, h);
  ...
  const _p3d_regRoom = function(
    ...
    if(z3d == null) z3d = 0.5;
    if(regScl == null) regScl = 1.0;
    // Have to test this value in game if {z3d} is larger than 1.0, not linear
    ...
    _p3d_room(x, y, z3d, w, h, color_gn, tryVal(z, Layer.power - 1.85));
    Draw.rect(reg, toP3dCoord(x, z3d, false), toP3dCoord(y, z3d, true), w * regFixScl, h * regFixScl);
  ...
  const _p3d_cylinderFade = function(
    ...
    let
      ...
      colorOut = Tmp.c3.set(colorIn);
    colorOut.a = 0.0;
    _p3d_cylinder(x, y, z3d, rad, colorIn, colorOut, z);
  };
  exports._p3d_cylinderFade = _p3d_cylinderFade;
