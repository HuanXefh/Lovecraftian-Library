...
*/
  ...
  const ters = (function() {
    ...
    MDL_event._c_onLoad(() => {
      Vars.content.blocks().each(
        blk => tryFun(blk.ex_getTags, blk, Array.air).includes("blk-mat0flr"),
        blk => ters.pushUnique(tryFun(blk.ex_getMatGrp, blk, "none")),
      );
      ters.pull("none");
    });
  ...
  const _ter = function thisFun(t, size, checkR) {
    ...
    let ter;
    ts.forEachFast(ot => {
      ter = tryFun(ot.floor().ex_getMatGrp, ot.floor(), null);
