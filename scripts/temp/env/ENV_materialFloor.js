...
*/
  ...
  function comp_init(blk) {
    if(blk.overwriteVanillaProp) {
      ...
      if(blk.speedMultiplier.fEqual(1.0)) blk.speedMultiplier = DB_env.db["grpParam"]["floor"]["speed"].read(blk.ex_getMatGrp(), 1.0);
    };

    DB_env.db["grpParam"]["floor"]["extraSetter"].read(blk.ex_getMatGrp(), Function.air)(blk, blk.overwriteVanillaProp);
...
*/
  ...
  .setParam({
    ...
    matGrp: "none",
    // @PARAM: A list of random overlay tags, see {DB_env.db["map"]["randRegTag"]}. Will be replaced with regions.
    randRegs: [],
