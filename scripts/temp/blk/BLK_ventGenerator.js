...
*/
  ...
  function comp_setStats(blk) {
    ...
    blk.stats.add(TP_stat.blk_attrReq, newStatValue(tb => {
      tb.row();
      MDL_table.setDisplay_attr(tb, blk.attribute, oblk => MDL_cond._isVentBlock(oblk) && oblk.ex_getVentSize() === blk.size);
    }));
  };


  function comp_canPlaceOn(blk, t, team, rot) {
    return MDL_cond._isVentBlock(t.floor()) && t.floor().ex_getVentSize() === blk.size;
  };
