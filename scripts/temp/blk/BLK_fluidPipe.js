...
*/
  ...
  function comp_onDestroyed(b) {
    if(PARAM.secret_steelPipe && String(b.block.ex_getMatGrp()).equalsAny([
  ...
  function comp_updateTile(b) {
    if(b.block.ex_getIsShortCircuitPipe() && Mathf.chance(0.008)) {
      b.block.consPower.trigger(b);
    };

    if(TIMER.sec && b.isLeak && b.liquids.currentAmount() > 0.001) {
      MDL_pollution.addDynaPol(MDL_pollution._liqPol(b.liquids.current()) / 60.0);
    };
  };
