...
*/
  ...
  function comp_setStats(blk) {
    ...
    if(blk.healsBuilding) blk.stats.add(TP_stat.blk0misc_repairAmt, MDL_text._healText(
      blk.bHealAmt,
      blk.bHealPerc,
    ));
    if(blk.healsUnit) blk.stats.add(TP_stat.blk0misc_unitRepairAmt, MDL_text._healText(
      blk.unitHealAmt,
      blk.unitHealPerc,
    ));
  ...
  function comp_updateTile(b) {
    ...
    if(b.charge > b.block.reload && cond) {
      ...
      if(b.block.ex_getHealsBuilding()) {
        MDL_pos._it_bs(b.x, b.y, rad, b.team, ob => MDL_cond._canHeal(ob), ob => {
          FRAG_attack.heal(ob, (ob.maxHealth * b.block.ex_getBHealPerc() + b.block.ex_getBHealAmt()) * (1.0 + b.phaseHeat * b.block.phaseBoost) * b.efficiency, true);
        });
      };

      if(b.block.ex_getHealsUnit()) {
        MDL_pos._it_units(b.x, b.y, rad, b.team, ounit => MDL_cond._canHeal(ounit), ounit => {
          if(FRAG_attack.heal(ounit, (ounit.maxHealth * b.block.ex_getUnitHealPerc() + b.block.ex_getUnitHealAmt()) * (1.0 + b.phaseHeat * b.block.phaseBoost) * b.efficiency)) {
...
*/
  ...
  module.exports = [
    ...
    .setGetter("bHealAmt", "bHealPerc", "unitHealAmt", "unitHealPerc", "healsBuilding", "healsUnit"),
