...
*/
  ...
  function comp_updateTile(b) {
    ...
    if(b.efficiency < 0.0001 || !b.shouldConsume()) {
      ...
      b.ex_onRcRun();
      if(b.hasStopped) b.ex_onRcStoppedRun();
    };
  ...
  .setProp({
    ...
    addLiqBar: (tb, b, liq) => {
      tb.add(new Bar(
        ...
        () => MDL_cond._isAuxilliaryFluid(liq) && !MDL_cond._isNoCapAuxilliaryFluid(liq) ? Mathf.clamp(b.liquids.get(liq) / VAR.ct_auxCap) : (b.liquids.get(liq) / b.block.liquidCapacity),
      )).growX();
      tb.row();
    },
  });
