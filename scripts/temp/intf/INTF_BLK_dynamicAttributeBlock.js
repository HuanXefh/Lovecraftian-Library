...
*/
  ...
  function comp_setStats(blk) {
    ...
    if(blk.hasDynaAttrItm && !blk.ex_getDynaAttrBaseAmt_itm().fEqual(0.0)) {
      blk.stats.add(TP_stat.blk0fac_prodSpd, blk.ex_getDynaAttrBaseAmt_itm() / blk.ex_getCraftTime(), StatUnit.itemsSecond);
    };
    if(blk.hasDynaAttrLiq && !blk.ex_getDynaAttrBaseAmt_liq().fEqual(0.0)) {
      blk.stats.add(TP_stat.blk0fac_prodSpd, blk.ex_getDynaAttrBaseAmt_liq() * 60.0, StatUnit.liquidSecond);
    };
  ...
  function comp_setBars(blk) {
    ...
    blk.addBar("efficiency", b => new Bar(
      prov(() => Core.bundle.format("bar.efficiency", Math.round(b.ex_getDynaAttrEffc() * 100.0))),
      prov(() => Pal.lightOrange),
      () => Mathf.clamp(b.ex_getDynaAttrEffc()),
    ));
  };
  ...
  function comp_onProximityUpdate(b) {
    ...
    let tup = MDL_attr._dynaAttrTup(b.block.ex_getAttrRsMap(), b.dynaAttrTs, b.block.ex_getAttrMode());
    if(tup == null) {
      b.dynaAttrSum = 0.0;
      b.dynaAttrRs = null;
    } else {
