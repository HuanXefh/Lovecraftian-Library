...
*/
  ...
  function comp_setStats(blk) {
    if(isFinite(blk.durabCap) && blk.durabCap > 0.0) {
      blk.stats.add(TP_stat.blk0fac_durabTime, (blk.durabCap / 3600.0 * (blk.durabRegenFracMin + blk.durabRegenFracMax) * 0.5).roundFixed(2), StatUnit.minutes);
    };
  };
  ...
  function comp_setBars(blk) {
    blk.addBar("lovec-durability", b => new Bar(
      prov(() => Core.bundle.format("bar.lovec-bar-durability-amt", b.ex_getDurabFrac().perc(0))),
      prov(() => Pal.sap),
      () => Mathf.clamp(b.ex_getDurabFrac()),
    ));
  };


  function comp_created(b) {
    b.durabDecMtp = b.block.ex_getDurabDecMtp();
  };


  function comp_updateTile(b) {
    if(b.durabMode === "dec") {
      b.durabFrac -= (!isFinite(b.block.ex_getDurabCap()) || b.block.ex_getDurabCap() < 0.0001) ?
        0.0 :
        (1.0 / b.block.ex_getDurabCap() * b.edelta());
      ...
      if(b.health / b.maxHealth > 0.9999) {
        b.durabFrac = Mathf.lerp(b.block.ex_getDurabRegenFracMin(), b.block.ex_getDurabRegenFracMax(), Math.random());
        b.durabMode = "dec";
      };
    };
  };
  ...
  function comp_drawSelect(b) {
    if(b.durabMode !== "dec") {
      MDL_draw._d_textSelect(b, MDL_bundle._info("lovec", "text-require-repair"), false, b.block.ex_getDurabTextOffTy());
    };
  };
