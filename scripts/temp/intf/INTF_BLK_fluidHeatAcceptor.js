...
*/
  ...
  function comp_load(blk) {
    blk.fHeatReg = fetchRegion(blk, "-fluid-heat", "-heat");
    if(!blk.fHeatReg.found()) blk.fHeatReg = null;
  };


  function comp_setStats(blk) {
    if(isFinite(blk.heatRes)) blk.stats.add(TP_stat.blk0heat_heatRes, blk.heatRes, TP_stat.rs_heatUnits);
  };
  ...
  function comp_setBars(blk) {
    ...
    blk.addBar("lovec-fheat", b => new Bar(
      prov(() => Core.bundle.format("bar.lovec-bar-fluid-heat-amt", Strings.fixed(b.ex_getFHeatCur(), 2) + " " + TP_stat.rs_heatUnits.localized())),
      prov(() => Pal.lightOrange),
      () => Mathf.clamp(b.ex_getFHeatCur() / blk.heatRes),
    ));
  };
  ...
  function comp_updateTile(b) {
    ...
    if(TIMER.liq) b.fHeatCur = Mathf.lerpDelta(b.fHeatCur, b.fHeatTg, b.block.ex_getFHeatWarmupRate());
  ...
  function comp_draw(b) {
    ...
    MDL_draw._reg_heat(
      ...
      b.block.ex_getFHeatReg(),
      b.block.ex_getShouldRotFHeatReg() ? b.drawrot() : 0.0,
      b.block.size,
    );
  };
