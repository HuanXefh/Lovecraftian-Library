...
*/
    ...
    Groups.unit.each(
      ...
      unit => {
        ...
        if(PARAM.drawUnitReload) {
          let mtIds;
          for(let i = 0; i < 3; i++) {
            mtIds = DB_unit.db["param"]["reloadBarIds"]["off" + i].read(unit.type.name);
  ...
  .setProp({
    drawBaseBuildStats: b => {
      ...
      if(PARAM.drawUnitReload) {
        let hasReload = b.ex_getReloadFrac != null || DB_block.db["group"]["showReload"].includes(b.block.name);
  ...
  function evComp_damage_buildDamageDisplay(b, bul) {
    ...
    let mode = MDL_entity._bShield(b, true) > dmg ?
      "shield" :
      "health";
